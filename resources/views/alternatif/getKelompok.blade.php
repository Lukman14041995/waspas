                            ...
                            <form class="p-4 md:p-5" method="post" action="{{ route('data-alternatif.store') }}" id="myForm" enctype="multipart/form-data">
                                ...
                                <div class="grid gap-4 mb-4 grid-cols-2">
                                    <div class="col-span-2">
                                        <label for="nama_alternatif" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Kode Kelompok</label>
                                        <input type="text" name="kode_kelompok" id="kode_kelompok" value="{{$newCode}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
                                    </div>
                                </div>
                                <div class="grid gap-4 mb-4 grid-cols-2">
                                    <div class="col-span-2">
                                        <label for="nama_alternatif" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">No
                                            SK</label>
                                        <input type="text" name="no_sk" id="no_sk" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
                                    </div>
                                </div>

                                <div class="grid gap-4 mb-4 grid-cols-2">
                                    <div class="col-span-2">
                                        <label for="nama_alternatif" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama
                                            Kelompok</label>
                                ...
                                <div class="mb-4">
                                    <label for="total_luas_tanah" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Total Luas
                                        Tanah</label>
                                    <input type="text" name="total_luas_tanah" id="total_luas_tanah" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" readonly>
                                </div>
                                <div id="input-container">
                                    <div class="mb-4 grid gap-4 grid-cols-3">
                                        ...
                                        <!-- Luas Tanah -->
                                        <div class="col-span-1">
                                            <label for="luas_tanah" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Luas
                                                Tanah</label>
                                            <input type="text" name="luas_tanah[]" class="luas-tanah bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                        </div>

                                        <!-- Tombol Add More -->
                                        <div class="col-span-1 flex items-end">
                                            <!-- Tombol Add More -->
                                            <button type="button" id="add-more" class="text-white inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                                Add
                                ...
                                @php
                                $kriteria = App\Models\Kriteria::whereNotIn('nama_kriteria', ['No SK', 'Luas Tanah'])->get();
                                @endphp
                                @foreach ($kriteria as $item)
                                <div class="grid gap-4 mb-4 grid-cols-2">
                                    @if($item->karakter_kriteria == 1)
                                    <div class="col-span-2">
                                        <label for="nama_kriteria" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{$item->nama_kriteria}}</label>
                                        <input type="hidden" name="kode_kriteria[]" value="{{$item->kode_kriteria}}">
                                        <input type="hidden" name="karakter_kriteria[]" value="{{$item->karakter_kriteria}}">
                                        <input type="text" name="value_kriteria" id="nama_alternatif" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                    </div>
                                    @elseif($item->karakter_kriteria == 2)
                                    <div class="col-span-2">
                                        <label for="nama_alternatif" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{$item->nama_kriteria}}</label>
                                        <input type="hidden" name="kode_kriteria[]" value="{{$item->kode_kriteria}}">
                                        <input type="hidden" name="karakter_kriteria[]" value="{{$item->karakter_kriteria}}">
                                        @php
                                        $opsi = App\Models\childKriteria::where('kode_kriteria', $item->kode_kriteria)->get();
                                        @endphp

                                        <select name="id_opsi[]" id="nama_alternatif" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                            @foreach ($opsi as $index => $opsiItem)
                                            <option value="{{$opsiItem->id}}">{{$opsiItem->jawaban}}</option>
                                            @endforeach
                                        </select>
                                    </div>
        ...
        <table class="w-full text-sm">
            <thead class="text-xs text-primary dark:text-purple uppercase bg-purple dark:bg-table-head">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3">
                        Luas Tanah Per Anggota
                    </th>
                    <th scope="col" class="px-6 py-3">
                        Total Luas Tanah Kelompok
                    </th>
                    <th scope="col" class="px-6 py-3">
                        Action
                    </th>
            ...
            <tbody class="text-xs md:text-base">
                ...
                <tr class="bg-gray-100 dark:bg-primary border-primary dark:border-purple text-primary dark:text-purple">
                    ...
                    @php
                    $anggota = App\Models\Anggota::where('kode_alternatif', $Alternatif->kode_alternatif)->get();
                    @endphp
                    <td class="text-center py-4">

                        @foreach($anggota as $agt)
                        <p style="text-align: left;">-{{$agt->nama_anggota}}</p>
                        @endforeach
                    ...
                    <td class="text-center py-4">
                        ...
                        @foreach($anggota as $agt)
                        <p style="text-align: left"> -{{$agt->luas_tanah}} m2</p>
                        @endforeach

                    </td>
                    <td class="text-center py-4">
                        {{ $Alternatif->luas_tanah }} m2
                    </td>
                    <td class="text-center py-4">
