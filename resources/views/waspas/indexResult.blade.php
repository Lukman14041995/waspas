                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="text-xs md:text-base">
                            @foreach ($ranking as $rank => $idAlternatif)
                                <tr
                                    class="bg-gray-100 dark:bg-primary border-b border-primary dark:border-purple last:border-0 text-primary dark:text-purple">
                                    <td class="text-center py-4">{{ $rank + 1 }}</th>
                                    <td class="text-center py-4">{{ $alternatifNames[$idAlternatif] }}</th>

                                    <td class="text-center py-4">
                                        @php
                                            $alter = App\Models\Alternatif::where('id', $idAlternatif)->first();
                                            $anggota = App\Models\Anggota::where('kode_alternatif', $alter->kode_alternatif)->get();
                                        @endphp
